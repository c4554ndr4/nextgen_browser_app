...
/**
 * Maps the shaka locales to Kid Safe Browser's active ones
 * This allows us to know which locale files are actually needed
 * and which shaka locale needs to be activated for a given Kid Safe Browser one.
...
function getMappings(shakaLocales, freeTubeLocales) {
   ...
   * The first element is the Kid Safe Browser locale, the second one is the shaka-player one
   */
  const mappings = []
  ...
  mappings.push(
    ...
    // not sure why we have pt, pt-PT and pt-BR in the Kid Safe Browser locales

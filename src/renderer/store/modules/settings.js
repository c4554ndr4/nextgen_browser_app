...
/*
 * Due to the complexity of the settings module in Kid Safe Browser, a more
 ...
 * additional consultation with the development team is preferred
...
const state = {
  ...
  useBackendFiltering: false,
  contentFilterPrompt: 'child-safe',
  customFilterPrompt: '',
  searchAssistantPrompt: 'educational-focus',
  customSearchAssistantPrompt: '',
...
const sideEffectHandlers = {
  currentLocale: async ({ dispatch }, value) => {
    ...
    // Only support en-US now
    if (targetLocale !== 'en-US') {
      console.warn(`Locale "${targetLocale}" not supported, using en-US`)
      targetLocale = 'en-US'
    }

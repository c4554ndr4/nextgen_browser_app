...
import { getEnhancedSearchSuggestions } from '../../services/ai-search-autocomplete'
...
async function getSearchSuggestionsLocal(query) {
  ...
  try {
    // Get traditional suggestions from local API
    const traditionalSuggestions = await getLocalSearchSuggestions(query)
    
    // Enhance with AI-powered suggestions based on parent's prompt
    const enhancedSuggestions = await getEnhancedSearchSuggestions(query, traditionalSuggestions)
    
    searchSuggestionsDataList.value = enhancedSuggestions
  } catch (error) {
    console.error('Error getting local search suggestions:', error)
    // Fallback to traditional suggestions only
    try {
      searchSuggestionsDataList.value = await getLocalSearchSuggestions(query)
    } catch (fallbackError) {
      console.error('Fallback search suggestions also failed:', fallbackError)
      searchSuggestionsDataList.value = []
    }
  }
}
...
async function getSearchSuggestionsInvidious(query) {
  ...
  try {
    // Get traditional suggestions from Invidious
    const invidiousResponse = await getInvidiousSearchSuggestions(query)
    const traditionalSuggestions = invidiousResponse.suggestions

...
/*
  Mapping of Shaka localization keys for control labels to Kid Safe Browser shortcuts.
...
export default defineComponent({
  ...
  setup: function (props, { emit, expose }) {
    ...
    // Helper function to validate UI state
    function isUIValid() {
      return ui && 
             !isDestroying && 
             ui.configure && 
             typeof ui.configure === 'function' &&
             ui.getControls &&
             typeof ui.getControls === 'function'
    }
    ...
    function getPlayerConfig(format, useAutoQuality = false) {
      return {
        // PERFORMANCE OPTIMIZATION: Optimized buffering settings for faster startup
        streaming: {
          bufferingGoal: 30, // Reduced from 180 for faster startup
          rebufferingGoal: 0.02,
          bufferBehind: 60, // Reduced from 300 to save memory
          // PERFORMANCE OPTIMIZATION: Reduce startup delay
          startAtSegmentBoundary: false,
          // PERFORMANCE OPTIMIZATION: Optimize for low latency
          lowLatencyMode: false,
          // PERFORMANCE OPTIMIZATION: Reduce initial buffering time
          stallEnabled: true,
          stallThreshold: 1
        },
        manifest: {
          ...
          dash: {
            ...
            // PERFORMANCE OPTIMIZATION: Faster manifest parsing
            clockSyncUri: ''
          }
        },
        abr: {
          ...
          // PERFORMANCE OPTIMIZATION: Faster quality switching
          switchInterval: 2, // Reduced from default 8
          // PERFORMANCE OPTIMIZATION: More aggressive bandwidth estimation
        ...
        textDisplayFactory: () => {
          try {
            ...
            console.warn('Failed to create SimpleTextDisplayer, using default:', error)
            // Return undefined to use the default text displayer
            return undefined
          }
        }
      }
    ...
    function configureUI(firstTime = false) {
      // Check if UI is valid before configuration
      if (!isUIValid()) {
        console.warn('UI not valid for configuration')
        return
      }
      ...
      try {
        if (firstTime) {
          ...
          // Configure UI with first-time settings
          ui.configure(firstTimeConfig)
        } else {
          // Configure UI with updated settings
    ...
    function addUICustomizations() {
      // Check if ui is available before trying to customize it
      if (!ui) {
        console.warn('UI not available for customization')
        return
      }
    ...
    watch(uiConfig, (newValue, oldValue) => {
      if (newValue !== oldValue && isUIValid()) {
        configureUI()
      }
    })
    ...
    async function setLocale(locale) {
      // Check if ui is available before trying to set locale
      if (!ui) {
        console.warn('UI not available for locale setting')
        return
      }

      // For most of Kid Safe Browser's locales, there is an equivalent one in shaka-player,
    ...
    function handleCanPlay() {
      ...
      if (startInPip && props.format !== 'audio' && ui && ui.getControls().isPiPAllowed() && process.env.IS_ELECTRON) {
        startInPip = false
        const { ipcRenderer } = require('electron')
    ...
    function keyboardShortcutHandler(event) {
      ...
      if (event.shiftKey && event.key === '?') {
        ...
        if (ui && ui.getControls().isFullScreenEnabled()) {
    ...
    onMounted(async () => {
      ...
      // Add a small delay to ensure UI is fully initialized before configuring
      await new Promise(resolve => setTimeout(resolve, 50))
      
      // Final check before configuring UI
      if (!isUIValid()) {
        console.warn('UI not valid for final configuration')
        return
      }
      ...
      try {
        ...
        console.warn('Error calling configureUI(true):', error)
        // Don't throw the error to prevent component crashes
      }

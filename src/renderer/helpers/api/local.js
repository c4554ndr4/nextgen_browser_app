 ...
 * @param {boolean} options.safetyMode whether to hide mature content (defaults to true for kid safety)
 ...
 */
async function createInnertube({ withPlayer = false, location = undefined, safetyMode = true, clientType = undefined, generateSessionLocally = true } = {}) {
  let cache
  if (withPlayer) {
  ...
  return await Innertube.create({
    ...
    // That behavior is highly undesirable for Kid Safe Browser, as we want to create a new session every time to limit tracking.
...
export async function getLocalSearchSuggestions(query) {
  ...
  if (searchSuggestionsSession === null) {
    searchSuggestionsSession = await createInnertube({ safetyMode: true })
  }
...
export async function getLocalPlaylist(id) {
  const innertube = await createInnertube({ safetyMode: true })
...
export async function getLocalTrending(location, tab, instance) {
  if (instance === null) {
    const innertube = await createInnertube({ location, safetyMode: true })
 ...
 */
export async function getLocalSearchResults(query, filters, safetyMode = true) {
  // Always ensure safety mode is enabled, but respect explicit false if passed
  const safeModeEnabled = safetyMode !== false
  const innertube = await createInnertube({ safetyMode: safeModeEnabled })
...
export async function getLocalVideoInfo(id) {
  const webInnertube = await createInnertube({ withPlayer: true, generateSessionLocally: false, safetyMode: true })
  ...
  if (
    ...
    const webEmbeddedInnertube = await createInnertube({ clientType: ClientType.WEB_EMBEDDED, safetyMode: true })
...
export async function getLocalChannelId(url) {
  const innertube = await createInnertube({ safetyMode: true })
  const response = await innertube.resolveURL(url)

  if (response.payload.browseEndpoint) {
    return response.payload.browseEndpoint.browseId
  } else {
    return null
  }
}
...
export async function getLocalChannelVideos(id) {
  const innertube = await createInnertube({ safetyMode: true })
  return await innertube.getChannel(id)
}
...
export async function getLocalChannelLiveStreams(id) {
  const innertube = await createInnertube({ safetyMode: true })
  const response = await innertube.getChannel(id)

  // if the channel doesn't have a live tab, getChannel throws an error in the youtube.js library
  // so we have to handle it here until that's fixed

  if (response.has_live_streams) {
    return await response.getLiveStreams()
  }
...
export async function getLocalChannelCommunity(id) {
  ...
  if (response.has_community) {
    return await response.getCommunity()
  }
...
export function parseLocalCommunityPosts(posts) {
  ...
  // see: https://github.com/Kid Safe BrowserApp/Kid Safe Browser/issues/3252#issuecomment-1546675781

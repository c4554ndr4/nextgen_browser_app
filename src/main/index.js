...
import { setupAutoUpdater, checkForUpdatesOnStartup, stopUpdateCheck, installUpdateNow, enableMockUpdate, simulateMockUpdateDownload, checkForUpdates } from './updater'
...
function runApp() {
  ...
  app.removeAsDefaultProtocolClient('kidsafebrowser')

  // If we are running a non-packaged version of the app && on windows
  if (process.env.NODE_ENV === 'development' && process.platform === 'win32') {
    // Set the path of electron.exe and your app.
    // These two additional parameters are only available on windows.
    app.setAsDefaultProtocolClient('kidsafebrowser', process.execPath, [path.resolve(process.argv[1])])
  } else {
    app.setAsDefaultProtocolClient('kidsafebrowser')
  }
  ...
  app.on('ready', async (_, __) => {
    ...
    // Kid Safe Browser only needs a few permissions, so we reject requests for other permissions
    // and reject all requests on non-Kid Safe Browser URLs.
    //
    // Kid Safe Browser needs the following permissions:
    ...
    session.defaultSession.setPermissionCheckHandler((webContents, permission, requestingOrigin) => {
      if (!isKidSafeBrowserUrl(requestingOrigin)) {
        return false
      }
    ...
    session.defaultSession.setPermissionRequestHandler((webContents, permission, callback) => {
      if (!isKidSafeBrowserUrl(webContents.getURL())) {
    ...
    // Initialize the updater after mainWindow is created
    if (process.env.NODE_ENV !== 'development') {
   ...
   */
  function isKidSafeBrowserUrl(urlString) {
  ...
  async function createWindow(
    ...
    const commonBrowserWindowOptions = {
      ...
      webPreferences: {
        ...
        // PERFORMANCE OPTIMIZATION: Enable hardware acceleration and optimize rendering
        enableRemoteModule: false,
        experimentalFeatures: true,
        // REMOVED: preload script - not needed with contextIsolation: false and nodeIntegration: true
        // PERFORMANCE OPTIMIZATION: Optimize memory usage
        v8CacheOptions: 'code',
        // PERFORMANCE OPTIMIZATION: Enable spellcheck caching
        spellcheck: false
      },
      minWidth: 340,
      minHeight: 380,
      // PERFORMANCE OPTIMIZATION: Show window faster
      show: false, // Always start hidden for faster perceived startup
      // PERFORMANCE OPTIMIZATION: Optimize window creation
  ...
  ipcMain.handle(IpcChannels.GET_SCREENSHOT_FALLBACK_FOLDER, (event) => {
    if (!isKidSafeBrowserUrl(event.senderFrame.url)) {
      return
    }

    return path.join(app.getPath('pictures'), 'Kid Safe Browser')
  })

  ipcMain.on(IpcChannels.CHOOSE_DEFAULT_FOLDER, async (event, kind) => {
    if (!isKidSafeBrowserUrl(event.senderFrame.url) || (kind !== DefaultFolderKind.DOWNLOADS && kind !== DefaultFolderKind.SCREENSHOTS)) {
      return
    }
  ...
  ipcMain.handle(IpcChannels.WRITE_TO_DEFAULT_FOLDER, async (event, kind, filename, arrayBuffer) => {
    if (
      !isKidSafeBrowserUrl(event.senderFrame.url) ||
    ...
    if (typeof currentPath === 'string' && folderPath.value.length > 0) {
      ...
      directory = path.join(app.getPath(kind === DefaultFolderKind.DOWNLOADS ? 'downloads' : 'pictures'), 'Kid Safe Browser')
    }
  ...
  // *********** //
  // Update Installation
  ipcMain.handle(IpcChannels.INSTALL_UPDATE_NOW, () => {
    installUpdateNow()
  })

  // Manual Update Check
  ipcMain.handle('check-for-updates-manually', () => {
    checkForUpdates(true) // true indicates manual check
  })

  // Development: Mock Update Testing (only in development)
  if (process.env.NODE_ENV === 'development') {
    ipcMain.handle('enable-mock-update', (_, version) => {
      enableMockUpdate(version)
    })

    ipcMain.handle('simulate-mock-update-download', () => {
      simulateMockUpdateDownload()
    })
  }
  ...
  /*
   * Callback when processing a kidsafebrowser:// link (macOS)
   */
   ...
   * Remove kidsafebrowser:// protocol if present
   */
  ...
  function baseUrl(arg) {
    let newArg = arg.replace('kidsafebrowser://', '')
    // add support for authority free url
      .replace('kidsafebrowser:', '')

    // fix for Qt URL, like `kidsafebrowser://https//www.youtube.com/watch?v=...`
    // For details see https://github.com/Kid Safe BrowserApp/Kid Safe Browser/pull/3119
  ...
  // PERFORMANCE OPTIMIZATION: Defer non-critical startup operations
  function deferNonCriticalStartup() {
    // Use setImmediate to defer operations until after the main window is created
    setImmediate(() => {
      // Setup auto updater after main window is ready
      setupAutoUpdater()
      
      // Check for updates after a delay to not impact startup performance
      setTimeout(() => {
